...
const WifiButtonToggle = () => {
    ...
    return (
        <button
            className={bind(network.wifi, "enabled").as((v) =>
                v ? "active" : ""
            )}
            onClick={() => {
...
const BluetoothButtonToggle = () => {
    ...
    const bluetoothState = Variable.derive(
        [bind(bluetooth, "isPowered")],
        (isPowered) => isPowered
    );
    return (
        <button
            className={bind(bluetoothState).as((v) => (v ? "active" : ""))}
            onClick={() => {
                exec(
                    `bluetoothctl power ${bluetoothState.get() ? "off" : "on"}`
                );
            }}
            cursor={"hand1"}
            onDestroy={() => bluetoothState.drop()}
        >
...
const AirplaneModeButtonToggle = () => {
    ...
    execAsync("rfkill list | sed -n 5p").then((v) =>
        airplaneMode.set(v.includes("yes"))
    );

    return (
        <button
            className={bind(airplaneMode).as((v) => (v ? "active" : ""))}
            onClick={() => {
...
const AdjustsColorScreenButtonToggle = () => {
    ...
    return (
        <button
            className={bind(gammastep).as((v) => (v ? "active" : ""))}
            onClick={() => {
                execAsync(
                    `${
                        gammastep.get()
...
const DarkModeButtonToggle = () => {
    const darkMode = Variable(
        exec("gsettings get org.gnome.desktop.interface gtk-theme").includes(
            "phocus"
        )
    );
    return (
        <button
            className={bind(darkMode).as((v) => (v ? "active" : ""))}
            onClick={() => {
                execAsync(
                    `${
                        darkMode.get()
...
const SlienceNotification = () => {
    ...
    return (
        <button
            className={bind(notifd, "dontDisturb").as((v) =>
                v ? "active" : ""
            )}
            onClick={() => {
...
const CloudflareTunnelButton = () => {
    const cloudflareTunnel = Variable(
        exec("warp-cli status").includes("Connected")
    );
    return (
        <button
            className={bind(cloudflareTunnel).as((v) => (v ? "active" : ""))}
            onClick={() => {
                exec(
                    `warp-cli ${
                        cloudflareTunnel.get() ? "disconnect" : "connect"
                    }`
                );
            ...
            cursor={"hand1"}
            onDestroy={() => cloudflareTunnel.drop()}
        >
            <icon
                icon={bind(cloudflareTunnel).as((v) =>
                    v ? "cloudflare-active" : "cloudflare"
                )}
            />
        </button>
    );

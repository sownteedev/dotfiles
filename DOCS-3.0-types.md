...
- [x] Quickshell
    ...
    - [x] PanelWindow
    - [x] PersistentProperties — giữ state DND và Caffeine qua hot reload.
    - ⛔ PopupAdjustment
...
- [x] Quickshell.Io
    ...
    - [ ] JsonAdapter — có thể thay một số JSON parsing thủ công nếu schema ổn định.
    - [ ] JsonObject
    - [x] Process
    - ⛔ Socket
    - ⛔ SocketServer
...
- [x] Quickshell.Networking
    ...
    - [x] ConnectionState
    - [x] DeviceType
    - [ ] NMSettings — IPv4/DNS hiện vẫn dùng `nmcli` vì cần chỉnh cấu hình đầy đủ.
    - [x] Network
    - ⛔ NetworkBackendType
...
1. `JsonAdapter`/`JsonObject`: chỉ nên dùng khi muốn chuẩn hóa các state JSON có schema cố định.

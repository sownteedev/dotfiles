...
use super::model::{ProcessMemoryDetails, ProcessSample, ProcessSnapshot, RankedProcess};
...
pub fn process_name(pid: u32, reported_name: &str) -> String {
    ...
    if electron_process && let Ok(environment) = fs::read(format!("/proc/{pid}/environ")) {
    ...
    if electron_process
        && let Ok(command_line) = fs::read(format!("/proc/{pid}/cmdline"))
        && let Some(app_name) = electron_app_name(&command_line)
    {
        return app_name;
    }
...
mod process_name_tests {
    use super::super::model::{ProcessSample, ProcessSnapshot};
    use super::{

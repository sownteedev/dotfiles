...
use super::json;
use super::nvidia::NvidiaReader;
use super::reader::read_text;
use std::fmt::Write;
use std::fs;
use std::io;
...
impl BatteryReader {
    ...
    pub fn encode(&self) -> String {
        ...
        json::escape_into(&device_name, &mut output);
        output.push_str("\"}");
        output
    }
}
...
pub fn encode_control() -> String {
    ...
    let start_threshold = start_threshold_path.as_ref().and_then(read_u8);
    let end_threshold = end_threshold_path.as_ref().and_then(read_u8);

//! System telemetry, battery, and process management for the core daemon.
...
pub use model::{ProcessMemoryDetails, ProcessMode, StatsPayload};

use model::{GpuStats, ProcessSnapshot};
use nvidia::NvidiaReader;
use reader::SystemReader;
...
impl Sampler {
    ...
    pub fn sample(&mut self, now: Instant) -> StatsPayload {
        let elapsed = self
            .previous_time
...
pub fn battery_control_json() -> String {
    battery::encode_control()
}

pub fn encode_stats(payload: &StatsPayload) -> String {
    json::encode(payload)
}

pub fn process_memory_details(pid: u32) -> io::Result<ProcessMemoryDetails> {
    SystemReader::new().process_memory_details(pid)
}

pub fn process_memory_details_json(details: &ProcessMemoryDetails) -> String {
    json::encode_process_memory(details)
}

pub fn terminate_process_tree(pid: u32) -> io::Result<()> {
    reader::terminate_process_tree(pid)
}
...
pub fn run_cli<I>(arguments: I) -> io::Result<()>
where
    I: IntoIterator<Item = String>,
{
    ...
    match arguments.next().as_deref() {
        ...
        Some("--process-memory") => {
            ...
            let details = process_memory_details(pid)?;

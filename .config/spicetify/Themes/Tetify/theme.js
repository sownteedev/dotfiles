...
(function bloom() {
	...
	const interval = setInterval(() => {
		...
		let playlistPlayLabel =
			Spicetify.Platform.Translations["playlist.a11y.play"];
		playlistPlayLabel = cleanLabel(playlistPlayLabel);
		let playlistPauseLabel =
			Spicetify.Platform.Translations["playlist.a11y.pause"];
		playlistPauseLabel = cleanLabel(playlistPauseLabel);

		const tracklistPlayLabel =
			Spicetify.Platform.Translations["tracklist.a11y.play"];
	...
	function updateLyricsPageProperties() {
		...
		function revealLyricsLines() {
			const lyricsLines = document.getElementsByClassName(
				"lyrics-lyricsContent-lyric",
			);
			let positionIndex = 0;
		...
		async function lyricsCallback(mutationsList) {
			for (let i = 0; i < mutationsList.length; i += 1) {
				for (let a = 0; a < mutationsList[i].addedNodes?.length; a += 1) {
					if (
						mutationsList[i].addedNodes[a].classList?.contains(
	...
	function addCategoryCardBackdrop() {
		waitForElement([".x-categoryCard-image"], () => {
			...
			const cardImages = document.getElementsByClassName(
	...
	function onPageChange() {
		const lyricsBackdropContainer = document.getElementById(
	...
	waitForHistoryAPI(() => {
		...
		waitForElement([".Root__lyrics-cinema"], () => {
			const lyricsCinema = document.getElementsByClassName(
				"Root__lyrics-cinema",
			)[0];

...
QtObject {
    ...
    property Process airpodsBatteryProcess: Process {
        command: [Config.sownteeshellDir + "/scripts/connectivity/airpods-battery-monitor.sh", root.airpodsAddress]
    ...
    function probeNearbyDevices(devices) {
        if (nearbyProbeProcess.running)
            return;
        var command = [Config.sownteeshellDir + "/scripts/connectivity/bluetooth-proximity-probe.sh"];
        for (var i = 0; i < devices.length; ++i) {
